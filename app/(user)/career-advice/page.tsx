...
export default async function page() {
  const query = groq`*[_type == 'post' && categories[]->title match 'Career Advice']{
    ...,
    author->,
    categories[]->
  }`;

  const advicePost: Post[] = await client.fetch(query);

  const BlogPosts = advicePost.map((blogPost) => {
  ...
  return (
    <section className="container mx-auto md:px-20 pb-10">
      <h1 className="font-bold text-4xl py-12 text-center">Career Advice</h1>
