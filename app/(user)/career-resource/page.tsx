...
export default async function page() {
  const query = groq`*[_type == 'post' && categories[]->title match 'Career Resource']{
    ...,
    author->,
    categories[]->
  }`;

  const resourcePost: Post[] = await client.fetch(query);

  const BlogPosts = resourcePost.map((blogPost) => {
  ...
  return (
    <section className="container mx-auto md:px-20 pb-10">
      <h1 className="font-bold text-4xl py-12 text-center">Career Resource</h1>
